...
for i in lst[:1]:
    ...
    # fig = px.scatter(new_df[i[0]], new_df[i[1]], color=new_df['cluster'], title=str(i[0])+'    V/S    '+str(i[1])).update_layout(xaxis_title=str(i[0]), yaxis_title=str(i[1]))
        ...
        fig = px.scatter(new_df['NPROCS'], new_df['TOTAL_IO_TIME'], color=new_df['cluster_colours'], color_discrete_sequence=px.colors.qualitative.Dark24, title=str(i[0])+'    V/S    '+str(i[1]))
